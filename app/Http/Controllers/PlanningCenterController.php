...
use App\Models\User;
use App\Helpers\CRM;
use App\Services\PlanningService as PlanningService;
        ...
        try {
            ...
            if (!is_null($contact)) {
                ...
                if ($contact->name != $lead['firstName'] || $contact->email != $lead['email'] || $contact->phone != $lead['phone'] || $contact->address != $lead['address1']) {
                    ...
                    $person_rs = $this->planningService->planning_api_call($linked . 'people' . $contact->people_id, 'PATCH', json_encode($perosn), [], true);
                    ...
                    if ($person_rs &&  property_exists($person_rs, 'data')) {
                        ...
                        if (!empty($emailr)) {
                            ...
                            $email_rs = $this->planningService->planning_api_call($linked . 'emails/' . $contact->email_id, 'PATCH', json_encode($email), [], true);
                        }
                        ...
                        if (!empty($phoner)) {
                            ...
                            $phone_rs = $this->planningService->planning_api_call($linked . 'phone_numbers/' . $contact->phone_id, 'PATCH', json_encode($phone), [], true);
                        }
                        ...
                        if (!empty($city) || !empty($state) || !empty($zip) || !empty($address1)) {
                            ...
                            $address_rs = $this->planningService->planning_api_call($linked . 'addresses/' . $contact->address_id, 'PATCH', json_encode($address), [], true);
                        }
                ...
                $person_res = $this->planningService->planning_api_call($linked . 'people', 'POST', json_encode($json), [], true);
                ...
                if ($person_res &&  property_exists($person_res, 'data')) {
                    ...
                    if (!empty($email)) {
                        ...
                        $email_rs = $this->planningService->planning_api_call($linked . $person_id . '/emails', 'POST', json_encode($email), [], true);
                    }

                    $user = loginUser(request()->user_id);
                    $workflow_selected =  $user->workflow_selected;
                    if (!empty($workflow_selected)) {
                        ...
                        $workflow = $this->planningService->planning_api_call('people/v2/workflows/' . $workflow_selected . '/cards', 'POST', json_encode($workflow), [], true);
                    }
                    ...
                    if (!empty($phone)) {
                        ...
                        $phone_rs = $this->planningService->planning_api_call($linked .  $person_id . '/phone_numbers', 'POST', json_encode($phone), [], true);
                    }
                    ...
                    if (!empty($city) || !empty($state) || !empty($zip) || !empty($address1)) {
                        ...
                        $address_rs = $this->planningService->planning_api_call($linked . $person_id . '/addresses', 'POST', json_encode($address), [], true);
                    }
    ...
    public function planning_lead_capture(Request $lead)
    {
        ...
        try {
            ...
            if ($dt->data->type == 'Person') {
                $res = CrmToken::where('company_id', $org_id)->first();
                $user = loginUser($res->user_id);
                $crm = $user->crmtoken;
                ...
                request()->location  = $res->user->location;
                ...
                if (!is_null($con)) {
                    ...
                    if ($con->name != $name || $con->email != $email || $con->phone != $phone || $con->address != $address) {
                        ...
                        // crmV2Loc($company_id = null, $location_id = null, $urlmain = '', $method = 'get', $data = '', $token = '', $json = true)
                        $response = CRM::crmV2($crm->company_id, 'contacts/' . $con->contact_id, 'PUT', $contacts, [], true, $crm->location_id, $crm);
                    ...
                    // save_logs($contacts);
                    $response = CRM::crmV2($crm->company_id, 'contacts/', 'POST', $contacts, [], true, $crm->location_id, $crm);
