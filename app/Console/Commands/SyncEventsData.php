...
class SyncEventsData extends Command
{
    ...
    {--location= : Sync for a specific location ID} {--created=} {--created_to=} {--type=} {--token=} {--updated=} {--all} {--location=} {--offset=}';
    ...
    public function handle(): int
    {
        ...
        if(!$token){
            ...
            try {
                $synced = $this->syncUserEvents($crm,$options );
            ...
            $progressBar->advance();
             ...
             try {
                $synced = $this->syncUserEvents($token, $options);
                $this->totalSynced += $synced;
            } catch (\Exception $e) {
        ...
        $progressBar->finish();
        $this->newLine(2);
        $this->info("✓ TOTAL: {$this->totalSynced} records synced across churches!");
    ...
    {
        $userId = $options['user'] ?? $crm->user_id??"";

        request()->user_id = $userId;
        $locationId = @$crm->location_id ?? 'unknown';
        $token  = $options['token'] ?? $crm->access_token??"";
        $syncedCount = 0;
        $offset = $options['offset'] ?? 0;
        $type = $options['type'] ?? '';
        $batch = [];

        $this->info("Syncing User: {$userId}");
        ...
        do {
            if ($type == 'headcount') {
                ...
                $response = $this->fetchAttendances($offset, $token, $filter);
            } else {
                $response = $this->fetchEventTimes($offset, $token);
            }
